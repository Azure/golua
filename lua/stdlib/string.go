...
import (
	...
	"github.com/Azure/golua/pkg/strs"
...
//
// The format string follows the same rules as the ISO C function sprintf. The only
// differences are that the options/modifiers *, h, L, l, n, and p are not supported
// and that there is an extra option, q.
//
// The q option formats a string between double quotes, using escape sequences when
// necessary to ensure that it can safely be read back by the Lua interpreter.
//
...
//
// When Lua is compiled with C89 compiler, options A and a (hex floats) do not support any
// modifier (flags, width, length).
//
...
func strFind(state *lua.State) int {
	var (
		subj  = state.CheckString(1)
		patt  = state.CheckString(2)
		index = int(state.OptInt(3, 1))
		plain = state.ToBool(4)
	)
	switch index = strs.StrPos(len(subj), index); {
		case index > len(subj) + 1:
			state.Push(nil)
			return 1
		case index < 1:
			index = 1
...
func strGsub(state *lua.State) int {
	var (
		subj = state.CheckString(1)
		patt = state.CheckString(2)
		repl = state.CheckString(3) // TODO: table, function
		upto = int(state.OptInt(4, int64(len(subj))))
	)
	s, n, err := strs.Gsub(subj, patt, repl, upto)
	if err != nil {
		panic(err)
	}
	state.Push(s)
...
func strRep(state *lua.State) int {
	...
	num := int(state.CheckInt(2))
	sep := state.OptString(3, "")
	state.Push(strs.Repeat(str, sep, num))
	return 1
}
